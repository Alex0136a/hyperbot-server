...
def init_db():
    ...
    try:
        ...
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_max_loss_pct REAL DEFAULT 0.3")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # Resserré de 1% à 0.3% à la demande explicite de l'utilisateur, suite à l'examen d'un
        # lot de clôtures Accumulation jugées trop tolérantes. Migre aussi les comptes encore
        # sur d'anciens défauts (0.5%, 0.7%) au cas où la migration précédente ne les aurait
        # pas atteints.
        conn.execute("UPDATE bot_config SET accumulation_max_loss_pct=0.3 WHERE accumulation_max_loss_pct IN (0.5, 0.7, 1.0)")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # Refonte du mécanisme de sortie Accumulation : Max Loss réduit à 0.5% (le SL exchange
        # reste inchangé, toujours x5 ce plafond — évite de reproduire le problème déjà corrigé
    ...
    try:
        # Mode SHORT en Accumulation — miroir du LONG (résistance au lieu de support, RSI
        # haut au lieu de bas) mais avec un switch, un plafond de positions et un levier
        # SÉPARÉS du LONG, comme demandé. Désactivé par défaut.
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_short_enabled INTEGER DEFAULT 0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_short_max_positions INTEGER DEFAULT 8")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_short_leverage INTEGER DEFAULT 2")
        conn.commit()
    except: pass
    try:
    ...
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_short_rsi_max REAL DEFAULT 85.0")
        conn.commit()
    except: pass
    try:
    ...
    try:
        ...
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_trailing_arm_pct REAL DEFAULT 0.5")
        conn.commit()
    except: pass
    try:
        conn.execute("UPDATE bot_config SET accumulation_trailing_arm_pct=0.5 WHERE accumulation_trailing_arm_pct=1.0")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # Palier PRINCIPAL du trailing Accumulation (distinct du palier précoce ci-dessus) —
        # architecture à deux paliers désormais IDENTIQUE au bot principal : le palier précoce
        # (accumulation_trailing_arm_pct, 0.5%) protège tôt mais se désactive dès que le pic
        # atteint ce palier principal (1%), qui prend seul la main avec son propre verrouillage
        # proportionnel au pic (accumulation_trailing_lock_ratio_pct).
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_trailing_main_pct REAL DEFAULT 1.0")
        conn.commit()
    except: pass
    try:
        # SORTIE SIMPLIFIÉE — seuil d'armement unique (0.4% par défaut), remplace l'ancien
        # armement à 1% (accumulation_trailing_main_pct, conservé mais plus utilisé par
        # Accumulation). Combiné au trailing proportionnel + plafond absolu déjà existants,
        # cette combinaison couvre naturellement tous les niveaux de pic (0.4%, 1%, 2%, 4%...).
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_arm_pct REAL DEFAULT 0.4")
        conn.commit()
    except: pass
    try:
        # Écart (en %) utilisé UNIQUEMENT par le palier précoce (entre le seuil d'armement et le
        # palier principal) — même valeur par défaut que le bot principal (0.42%), non élargi
        # (l'élargissement progressif reste réservé au palier principal une fois actif).
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_trailing_gap_pct REAL DEFAULT 0.42")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # ==================== MODE BREAKOUT — nouveau système indépendant ====================
        # Contrairement à Accumulation (retour à la moyenne, achète bas/vend haut dans un range),
        # Breakout est un système de MOMENTUM : détecte un range (même détection que la
        # suggestion RANGE informative) puis entre quand le prix CASSE le range, confirmé par un
        # dépassement du niveau + une expansion de volume. Toggles séparés LONG/SHORT, sizing et
        # levier dédiés, sortie par Max Loss + trailing (pas de "toucher de cible" — un breakout
        # n'a pas de niveau opposé prédéfini comme cible, contrairement à l'Accumulation).
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_enabled INTEGER DEFAULT 0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_short_enabled INTEGER DEFAULT 0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_max_positions INTEGER DEFAULT 5")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_long_leverage INTEGER DEFAULT 2")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_short_leverage INTEGER DEFAULT 2")
        conn.commit()
    except: pass
    try:
        # Multiplicateur de volume exigé sur la bougie de cassure vs moyenne des 20 dernières —
        # indépendant de volume_spike_mult (utilisé ailleurs pour le bot principal).
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_volume_mult REAL DEFAULT 1.5")
        conn.commit()
    except: pass
    try:
        # Marge de dépassement (%) au-delà du niveau pour confirmer une VRAIE cassure (pas
        # juste une mèche) — même principe que accumulation_breakdown_buffer_pct.
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_confirm_buffer_pct REAL DEFAULT 0.3")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_max_loss_pct REAL DEFAULT 0.3")
        conn.commit()
    except: pass
    try:
        conn.execute("UPDATE bot_config SET breakout_max_loss_pct=0.3 WHERE breakout_max_loss_pct IN (0.7, 1.0)")
        conn.commit()
    except: pass
    try:
    ...
    try:
        ...
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_trailing_arm_pct REAL DEFAULT 0.5")
        conn.commit()
    except: pass
    try:
        conn.execute("UPDATE bot_config SET breakout_trailing_arm_pct=0.5 WHERE breakout_trailing_arm_pct=1.0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_trailing_main_pct REAL DEFAULT 1.0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_trailing_gap_pct REAL DEFAULT 0.42")
        conn.commit()
    except: pass
    try:
    ...
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_trailing_lock_ratio_pct REAL DEFAULT 50.0")
        conn.commit()
    except: pass
    try:
        # Sélecteur paper/live dédié à Breakout — indépendant de manual_accum_trading_mode et de
        # accumulation_short_trading_mode (peut tourner en paper pendant que les autres sont live).
        conn.execute("ALTER TABLE bot_config ADD COLUMN breakout_trading_mode TEXT DEFAULT 'paper'")
        conn.commit()
    except: pass
    try:
    ...
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN range_trading_max_loss_pct REAL DEFAULT 0.3")
        conn.commit()
    except: pass
    try:
        conn.execute("UPDATE bot_config SET range_trading_max_loss_pct=0.3 WHERE range_trading_max_loss_pct=1.0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN range_trading_trailing_arm_pct REAL DEFAULT 0.5")
        conn.commit()
    except: pass
    try:
        conn.execute("UPDATE bot_config SET range_trading_trailing_arm_pct=0.5 WHERE range_trading_trailing_arm_pct=1.0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN range_trading_trailing_main_pct REAL DEFAULT 1.0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN range_trading_trailing_gap_pct REAL DEFAULT 0.42")
        conn.commit()
    except: pass
    try:
        # ==================== MODE SPOT ACCUMULATION — achat/hold/vente réel, PAS de levier,
        # PAS de Max Loss ==================== Achète l'actif réel (spot, stocké sur le wallet
        # Hyperliquid), le conserve jusqu'à ce que le prix redevienne favorable — aucune sortie
        # forcée en perte, contrairement à tous les autres modes. LONG uniquement (pas de short
        # en spot sans emprunt). Exige un canal large (2-3% par défaut, contre 0.5% pour
        # l'Accumulation à levier) — l'idée n'est pas de gratter un micro-mouvement mais de
        # capter un vrai va-et-vient avant de revendre.
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_enabled INTEGER DEFAULT 0")
        conn.commit()
    except: pass
    try:
    ...
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_min_channel_pct REAL DEFAULT 2.5")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_proximity_pct REAL DEFAULT 1.0")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # Une fois l'objectif atteint, un trailing s'arme et verrouille ce % du pic — laisse
        # courir une progression au-delà de l'objectif plutôt que de vendre pile dessus.
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_trailing_lock_ratio_pct REAL DEFAULT 50.0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_momentum_margin_pct REAL DEFAULT 0.3")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_candle_atr_mult REAL DEFAULT 2.0")
        conn.commit()
    except: pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_trading_mode TEXT DEFAULT 'paper'")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # NOUVELLE approche du trailing (remplace l'objectif basé sur le canal comme seuil
        # d'armement par défaut à l'achat) : armement à ce % fixe de PnL, indépendant de
        # l'amplitude du canal détecté. Le canal reste utilisé pour la DÉTECTION à l'achat,
        # plus pour l'objectif de sortie.
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_trailing_arm_pct REAL DEFAULT 1.0")
        conn.commit()
    except: pass
    try:
        # Plafond ABSOLU de PnL — vend immédiatement, sans attendre de confirmation de
        # retournement, dès que ce seuil est atteint. Évite d'attendre indéfiniment une
        # confirmation technique sur un très gros pic, où un aller-retour coûterait cher même
        # avec un plancher de pullback serré.
        conn.execute("ALTER TABLE bot_config ADD COLUMN spot_accum_hard_cap_pct REAL DEFAULT 20.0")
        conn.commit()
    except: pass
    try:
        # Plafonne les comptes déjà réglés à une valeur aberrante (ex: 50, censé être un
        # "stop loss à 50%" mais interprété comme 50% SOUS le support — rendant la protection
        # quasiment inopérante) vers une valeur raisonnable.
        conn.execute("UPDATE bot_config SET spot_accum_breakdown_buffer_pct=5.0 WHERE spot_accum_breakdown_buffer_pct > 5.0")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # NOUVEAU CHEMIN D'ENTRÉE — pendant la phase d'accumulation elle-même (vrai range
        # détecté), sans attendre qu'une tendance s'établisse ou qu'un début de mouvement se
        # produise. Volontairement plus strict que le simple bonus de confiance (1.15x) car ce
        # seuil CONTOURNE les exigences de tendance/mouvement plutôt que de les renforcer.
        conn.execute("ALTER TABLE bot_config ADD COLUMN accumulation_range_volume_mult REAL DEFAULT 1.5")
        conn.commit()
    except: pass
    try:
    ...
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN max_loss_pct REAL DEFAULT 0.3")
        conn.commit()
    except: pass
    try:
        conn.execute("UPDATE bot_config SET max_loss_pct=0.3 WHERE max_loss_pct=1.0")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # Miroir pour l'Accumulation SHORT : résistance de référence au moment de la vente à
        # découvert — si le prix la dépasse avec une marge de confirmation, on rachète même à
        # perte plutôt que d'attendre indéfiniment une résistance qui n'existe peut-être plus.
        conn.execute("ALTER TABLE paper_trades ADD COLUMN accumulation_resistance_price REAL")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # Marque un trade comme ouvert par le mode Breakout (mutuellement exclusif avec
        # is_accumulation) — sortie par Max Loss + trailing dédiés, pas de TP par défaut ni de
        # toucher de cible (un breakout n'a pas de niveau opposé prédéfini).
        conn.execute("ALTER TABLE paper_trades ADD COLUMN is_breakout INTEGER DEFAULT 0")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # Canal support/résistance détecté à l'ENTRÉE (%) — mémorisé sur le trade pour la
        # refonte de sortie Accumulation : un canal large (>2%) autorise une tolérance de
        # trailing plus généreuse (1% au lieu de 0.5%) une fois un pic suffisant atteint.
        conn.execute("ALTER TABLE paper_trades ADD COLUMN accumulation_channel_pct REAL")
        conn.commit()
    except: pass
    try:
    ...
    try:
        # Table DÉDIÉE au mode Spot Accumulation — délibérément SÉPARÉE de paper_trades, car
        # un holding spot n'a ni levier ni Stop Loss ni notion de "position" à liquider : c'est
        # un token réellement détenu sur le wallet, tenu jusqu'à ce que le prix redevienne
        # favorable, sans jamais de sortie forcée en perte.
        conn.execute("""CREATE TABLE IF NOT EXISTS spot_holdings (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            coin TEXT NOT NULL,
            status TEXT DEFAULT 'OPEN',
    ...
    try:
        ...
        pass
    try:
        # Plafond de pullback GLOBAL (4 modes de paper_trades — bot principal, Accumulation,
        # Breakout, Range Trading) : dès qu'un trade atteint ce seuil de pic (3% par défaut),
        # aucun mécanisme de trailing ne doit laisser reperdre plus que ce plafond (0.5 point de
        # % par défaut) — le plancher retenu est le PLUS STRICT entre celui déjà calculé par
        # chaque mode et ce plafond, même principe déjà en place pour Spot Accumulation
        # (spot_accum_trailing_max_giveback_pct), généralisé ici.
        conn.execute("ALTER TABLE bot_config ADD COLUMN ttp_giveback_cap_after_pct REAL DEFAULT 3.0")
        conn.commit()
    except Exception:
        pass
    try:
        conn.execute("ALTER TABLE bot_config ADD COLUMN ttp_giveback_cap_pct REAL DEFAULT 0.5")
        conn.commit()
    except Exception:
        pass
    try:
...
def hl_spot_coin_name(coin: str) -> str:
    ...
    Hyperliquid pour les info/exchange endpoints — 'TOKEN/USDC'. Le SDK Python résout ce nom
    vers l'asset ID interne (10000 + index dans spotMeta.universe) via son propre cache de
    métadonnées, chargé automatiquement à l'initialisation de l'objet Exchange/Info."""
    return f"{coin}/USDC"

def hl_spot_buy(account_address: str, coin: str, size_usdc: float, cur_price: float):
    """Achète l'actif RÉEL au comptant (spot) sur Hyperliquid — PAS de levier, PAS de SL posé
    sur l'exchange (le mode Spot Accumulation ne vend jamais à perte par conception, un SL de
    sécurité exchange n'aurait donc pas de sens ici). Retourne (qty, fill_price) ou lève une
    exception. AVERTISSEMENT : ce code suit la documentation officielle Hyperliquid mais n'a
    pas pu être testé contre l'API réelle depuis cet environnement — tester en Paper puis avec
    un tout petit montant en Live avant d'y engager un capital significatif."""
    exchange = get_hl_exchange(account_address)
    spot_coin = hl_spot_coin_name(coin)
    size_decimals = get_hl_size_decimals(coin)
    qty = round(size_usdc / cur_price, size_decimals)
    if qty <= 0:
    ...
    try:
        ...
        if statuses and "filled" in statuses[0]:
            fill_price = float(statuses[0]["filled"]["avgPx"])
            filled_qty = float(statuses[0]["filled"]["totalSz"])
    except Exception:
        pass
...
async def manage_open_trade(user_id: int, trade: dict, cur: float, conn, accum_reversal_confirmed: bool = None, candle_color: str = None):
    ...
    if trade.get("is_accumulation"):
        is_short_accum = trade["action"] == "SHORT"
        pnl_pct_live = pnl / trade["size_usdc"] * 100
        accum_close_reason = None
        accum_log_msg = None

        # SORTIE SIMPLIFIÉE — remplace l'ancien empilement (TP manuel + trailing + retournement
        # MACD 1h + cassure confirmée). Un seul armement à 0.4% de pic, un plancher qui combine
        # trailing proportionnel (50% du pic) et plafond absolu (jamais plus de 0.5pt redonné
        # après 3% de pic, voir _apply_giveback_cap) — cette combinaison couvre déjà TOUS les
        # niveaux de pic naturellement (0.4%, 1%, 2%, 4%, 8%...), pas seulement 3%. Une fois
        # armé, le trade continue de courir tant qu'AUCUN signal structurel ne se produit :
        # prix du mauvais côté de l'EMA200, OU bougie qui clôture dans la mauvaise couleur.
        ...
        cfg_trail2 = conn.execute("SELECT accumulation_arm_pct, accumulation_trailing_lock_ratio_pct FROM bot_config WHERE user_id=?", (user_id,)).fetchone()
        accum_arm_pct = cfg_trail2["accumulation_arm_pct"] if cfg_trail2 and "accumulation_arm_pct" in cfg_trail2.keys() and cfg_trail2["accumulation_arm_pct"] is not None else 0.4
        ...
        if accum_peak_pct >= accum_arm_pct:
            if not trade.get("trailing_armed"):
            ...
            if pnl_pct_live <= accum_floor:
                ...
                accum_log_msg = f"🔒 {trade['coin']}: Plancher touché (pic {round(accum_peak_pct,2)}%, plancher {round(accum_floor,2)}%) — {'rachat' if is_short_accum else 'revente'} {round(pnl,2)} USDC"
            else:
                # Signal structurel 1 : prix du mauvais côté de l'EMA200 (change très lentement,
                # cache par cycle de scan — voir ema200_by_coin).
                ema200_val = ema200_by_coin.get(trade["coin"])
                wrong_side_ema = False
                if ema200_val:
                    wrong_side_ema = (cur < ema200_val) if not is_short_accum else (cur > ema200_val)
                # Signal structurel 2 : la bougie close dans la mauvaise couleur (candle_color
                # fourni par l'appelant — temps réel via current_candle_open, ou None sur les
                # vérifications où cette donnée n'est pas disponible, auquel cas ce signal est
                # simplement ignoré ce tick-ci).
                wrong_candle_color = False
                if candle_color:
                    wrong_candle_color = (candle_color == "RED") if not is_short_accum else (candle_color == "GREEN")

                if wrong_side_ema:
                    accum_close_reason = "ACCUMULATION_TRAILING_TP"
                    accum_log_msg = f"📉 {trade['coin']}: prix repassé du mauvais côté de l'EMA200 (pic {round(accum_peak_pct,2)}%, armé) — {'rachat' if is_short_accum else 'revente'} {round(pnl,2)} USDC"
                elif wrong_candle_color:
                    accum_close_reason = "ACCUMULATION_TRAILING_TP"
                    accum_log_msg = f"🕯️ {trade['coin']}: bougie clôturée en couleur défavorable ({candle_color}, pic {round(accum_peak_pct,2)}%, armé) — {'rachat' if is_short_accum else 'revente'} {round(pnl,2)} USDC"

        # Filet ABSOLU — reste séparé et INCONDITIONNEL, catégorie différente (protection
        # contre une perte catastrophique, pas gestion du profit) — voir HARD_STOP_REASONS.
        ...
        if not accum_close_reason and pnl_pct_live <= -accum_abs_max_loss_pct:
            ...
            accum_log_msg = f"🛑 {trade['coin']}: Filet ABSOLU atteint ({round(pnl_pct_live,2)}% ≤ -{accum_abs_max_loss_pct}%) — {'rachat' if is_short_accum else 'revente'} {round(pnl,2)} USDC pour limiter la perte"
    ...
    # Mode BREAKOUT : sortie par Max Loss fixe + trailing (armé à partir d'un petit pic, verrouille
    # un % du pic) — même mécanique que le nouveau trailing Accumulation, réglages indépendants.
    # Pas de TP manuel, pas de toucher de cible (un breakout n'a pas de niveau opposé prédéfini).
    if trade.get("is_breakout"):
        pnl_pct_live = pnl / trade["size_usdc"] * 100
        bo_close_reason = None
        bo_log_msg = None

        cfg_bo = conn.execute("SELECT breakout_max_loss_pct, breakout_trailing_arm_pct, breakout_trailing_main_pct, breakout_trailing_lock_ratio_pct, breakout_trailing_gap_pct FROM bot_config WHERE user_id=?", (user_id,)).fetchone()
        bo_max_loss_pct = cfg_bo["breakout_max_loss_pct"] if cfg_bo and "breakout_max_loss_pct" in cfg_bo.keys() and cfg_bo["breakout_max_loss_pct"] is not None else 0.3
        bo_early_arm = cfg_bo["breakout_trailing_arm_pct"] if cfg_bo and "breakout_trailing_arm_pct" in cfg_bo.keys() and cfg_bo["breakout_trailing_arm_pct"] is not None else 0.5
        bo_main_arm = cfg_bo["breakout_trailing_main_pct"] if cfg_bo and "breakout_trailing_main_pct" in cfg_bo.keys() and cfg_bo["breakout_trailing_main_pct"] is not None else 1.0
        ...
        bo_trail_gap = cfg_bo["breakout_trailing_gap_pct"] if cfg_bo and "breakout_trailing_gap_pct" in cfg_bo.keys() and cfg_bo["breakout_trailing_gap_pct"] is not None else 0.42

        if pnl_pct_live <= -bo_max_loss_pct:
            bo_close_reason = "BREAKOUT_MAX_LOSS"
            bo_log_msg = f"🛑 {trade['coin']}: Breakout Max Loss atteint ({round(pnl_pct_live,2)}% <= -{bo_max_loss_pct}%) — clôture, {round(pnl,2)} USDC"

        bo_peak_pct = float(trade["peak_price_pct"]) if trade.get("peak_price_pct") is not None else 0.0
        if pnl_pct_live > bo_peak_pct:
            bo_peak_pct = pnl_pct_live
            conn.execute("UPDATE paper_trades SET peak_price_pct=? WHERE id=?", (bo_peak_pct, trade["id"]))
        ...
        elif not bo_close_reason and bo_peak_pct >= bo_early_arm:
            bo_early_floor = round(bo_peak_pct - bo_trail_gap, 4)
            if pnl_pct_live <= bo_early_floor:
                bo_close_reason = "BREAKOUT_EARLY_FLOOR"
        ...
        if bo_close_reason:
            ...
            if is_live_bo:
                ...
                try:
                    hl_close_position(account_address, trade["coin"], trade.get("hl_sl_oid"))
                    add_bot_log(user_id, f"🔴 {trade['coin']}: position réelle fermée sur Hyperliquid ({bo_close_reason})", "success")
                except Exception as e:
                    add_bot_log(user_id, f"⛔ {trade['coin']}: ÉCHEC de fermeture réelle sur Hyperliquid — {e} — position gardée OUVERTE côté suivi, nouvelle tentative au prochain cycle", "error")
                    close_confirmed = False
            if close_confirmed:
                conn.execute("""UPDATE paper_trades SET status='CLOSED', current_price=?, pnl=?, pnl_pct=?,
                closed_at=?, close_reason=? WHERE id=?""",
                (cur, round(pnl,2), round(pnl_pct_live,2), datetime.utcnow().isoformat(), bo_close_reason, trade["id"]))
                if not is_live_bo:
                    conn.execute("UPDATE paper_portfolio SET balance=balance+?+? WHERE user_id=?",
                                 (trade["size_usdc"], round(pnl,2), user_id))
                conn.commit()
    ...
    if trade.get("is_range_trade"):
        ...
        cfg_rt = conn.execute("SELECT range_trading_max_loss_pct, range_trading_trailing_arm_pct, range_trading_trailing_main_pct, range_trading_trailing_lock_ratio_pct, range_trading_trailing_gap_pct FROM bot_config WHERE user_id=?", (user_id,)).fetchone()
        rt_max_loss_pct = cfg_rt["range_trading_max_loss_pct"] if cfg_rt and "range_trading_max_loss_pct" in cfg_rt.keys() and cfg_rt["range_trading_max_loss_pct"] is not None else 0.3
        rt_early_arm = cfg_rt["range_trading_trailing_arm_pct"] if cfg_rt and "range_trading_trailing_arm_pct" in cfg_rt.keys() and cfg_rt["range_trading_trailing_arm_pct"] is not None else 0.5
        rt_main_arm = cfg_rt["range_trading_trailing_main_pct"] if cfg_rt and "range_trading_trailing_main_pct" in cfg_rt.keys() and cfg_rt["range_trading_trailing_main_pct"] is not None else 1.0
        ...
        rt_trail_gap = cfg_rt["range_trading_trailing_gap_pct"] if cfg_rt and "range_trading_trailing_gap_pct" in cfg_rt.keys() and cfg_rt["range_trading_trailing_gap_pct"] is not None else 0.42
        ...
        elif not rt_close_reason and rt_peak_pct >= rt_early_arm:
            rt_early_floor = round(rt_peak_pct - rt_trail_gap, 4)
            if pnl_pct_live <= rt_early_floor:
                rt_close_reason = "RANGE_TRADE_EARLY_FLOOR"
    ...
    max_loss_pct = pick("custom_max_loss_pct", "max_loss_pct", 0.3)
...
async def manage_spot_holdings(user_id: int, prices: dict, candle_color_by_coin: dict = None):
    """Gère les holdings Spot Accumulation ouverts — Stop Loss simple et inconditionnel (séparé,
    catégorie différente : protection contre une perte catastrophique, pas gestion du profit),
    puis SORTIE SIMPLIFIÉE (même design qu'Accumulation, voir manage_open_trade) : armement
    unique à 0.4% de pic, plancher combinant trailing proportionnel (50% du pic) et plafond
    absolu (jamais plus de 0.5pt redonné après 3% de pic), puis une fois armé le holding
    continue de courir tant qu'aucun signal structurel ne se produit (prix du mauvais côté de
    l'EMA200, ou bougie qui clôture en rouge).

    ANTI-MÈCHE (tous déclencheurs sauf le Stop Loss dur) : même mécanisme que manage_open_trade
    — voir anti_wick_check. candle_color_by_coin n'est fourni qu'au cycle de scan ; ema200_by_coin
    est une variable GLOBALE du module, accessible directement sans être passée en paramètre."""
    conn = get_db()
    ...
    for h in holdings:
        h = dict(h)
        cur = prices.get(h["coin"])
        if not cur:
            continue
        pnl_pct = (cur - h["entry_price"]) / h["entry_price"] * 100
        ...
        close_reason = None
        # Stop Loss SIMPLE et INCONDITIONNEL — reste séparé, catégorie différente (protection
        # contre une perte catastrophique, pas gestion du profit). Vérifié en priorité.
        if pnl_pct <= -spot_max_loss_pct:
            close_reason = "SPOT_ACCUM_MAX_LOSS"

        # SORTIE SIMPLIFIÉE — même design qu'Accumulation (voir manage_open_trade) : un seul
        # armement à 0.4% de pic, un plancher qui combine trailing proportionnel (50% du pic)
        # et plafond absolu (jamais plus de 0.5pt redonné après 3% de pic) — couvre déjà tous
        # les niveaux de pic naturellement. Une fois armé, le holding continue de courir tant
        # qu'AUCUN signal structurel ne se produit : prix du mauvais côté de l'EMA200, OU
        # bougie qui clôture dans la mauvaise couleur (LONG uniquement sur ce mode).
        trailing_armed = bool(h["trailing_armed"])
        if not close_reason:
            spot_arm_pct = 0.4
            if peak_pct >= spot_arm_pct and not trailing_armed:
            ...
            if trailing_armed:
                ...
                if pnl_pct <= floor_pct:
                    ...
                    add_bot_log(user_id, f"🔒 {h['coin']}: Plancher touché (pic {round(peak_pct,2)}%, plancher {round(floor_pct,2)}%) — revente {round(pnl,2)} USDC", "success")
                else:
                    ema200_val_sp = ema200_by_coin.get(h["coin"])
                    wrong_side_ema_sp = bool(ema200_val_sp and cur < ema200_val_sp)
                    candle_color_h_early = candle_color_by_coin.get(h["coin"]) if candle_color_by_coin else None
                    wrong_candle_color_sp = bool(candle_color_h_early == "RED")

                    if wrong_side_ema_sp:
                        close_reason = "SPOT_ACCUM_TARGET_TRAILING"
                        add_bot_log(user_id, f"📉 {h['coin']}: prix repassé sous l'EMA200 (pic {round(peak_pct,2)}%, armé) — revente {round(pnl,2)} USDC", "success")
                    elif wrong_candle_color_sp:
                        close_reason = "SPOT_ACCUM_TARGET_TRAILING"
                        add_bot_log(user_id, f"🕯️ {h['coin']}: bougie clôturée en rouge (pic {round(peak_pct,2)}%, armé) — revente {round(pnl,2)} USDC", "success")
        ...
        if close_reason:
            ...
            if close_confirmed:
                ...
                conn.execute("""UPDATE spot_holdings SET status='CLOSED', current_price=?, pnl=?, pnl_pct=?,
                    closed_at=?, close_reason=? WHERE id=?""",
                    (fill_price_sell, round(final_pnl, 4), round(final_pnl_pct, 4), datetime.utcnow().isoformat(), close_reason, h["id"]))
                if not is_live_h:
                    conn.execute("UPDATE paper_portfolio SET balance=balance+?+? WHERE user_id=?",
                ...
                # SPOT_ACCUM_TARGET_TRAILING : message déjà loggé au moment de la détection du
                # déclencheur précis (plancher / EMA200 / couleur de bougie), pas de doublon ici.
        conn.close()
...
async def scan_markets(user_id: int):
    ...
    async with httpx.AsyncClient() as client:
        ...
        for coin in coins_to_scan:
            ...
            # EMA200 par coin — calculée une fois par cycle (comme la couleur de bougie),
            # réutilisée en temps réel pour la nouvelle sortie simplifiée Accumulation/Spot
            # Accumulation (change très lentement, pas besoin de la recalculer à chaque tick).
            ema200_val = calc_ema(closes, 200)
            ema200_by_coin[coin] = ema200_val[-1] if ema200_val else None
            ...
            # ==================== MODE BREAKOUT — détection (LONG/SHORT) ====================
            # Réutilise support/resistance/channel_pct déjà calculés ci-dessus pour RANGE — même
            # source de données, mais critère de déclenchement opposé : au lieu d'attendre un
            ...
            if config and (config["breakout_enabled"] or config["breakout_short_enabled"]) and coin not in breakout_blocked_set and resistance and support:
                ...
                if config["breakout_enabled"]:
                    ...
                    if price >= breakout_up_level and vol_expansion:
                        ...
                        add_bot_log(user_id, f"🚀🔍 {coin}: cassure de résistance ${resistance:.4g} en cours, mais volume insuffisant ({vol_cur/max(vol_avg,0.0001):.2f}x < {vol_mult_bo}x requis) — pas d'achat", "info")
                ...
                if config["breakout_short_enabled"]:
                    ...
                    if price <= breakout_down_level and vol_expansion:
                        ...
                        add_bot_log(user_id, f"🚀🔍 {coin}: cassure de support ${support:.4g} en cours, mais volume insuffisant ({vol_cur/max(vol_avg,0.0001):.2f}x < {vol_mult_bo}x requis) — pas de vente", "info")
                    ...
                    if not near_support:
                        ...
                        if entry_score < 2:
                            ...
                            if should_log_diag:
                                ...
                                add_bot_log(user_id, f"💰🔍 {coin}: score suffisant ({entry_score}/3) mais momentum pas cohérent ({raison_mom}) — pas d'achat (évite de rattraper un couteau qui tombe)", "info")
                            ...
                            if should_log_diag:
                                ...
                                add_bot_log(user_id, f"💰🔍 {coin}: score suffisant ({entry_score}/3) mais ATR ({round(atr_pct,3) if atr_pct else '?'}%) hors plage cohérente avec le SL ({raison_atr}) — pas d'achat", "info")
                        elif accumulation_coin_recently_closed(user_id, coin, "LONG"):
                            add_bot_log(user_id, f"⏳ {coin}: score {entry_score}/3 confirmé mais une position Accumulation LONG vient d'être fermée sur ce coin — cooldown, pas de rachat immédiat", "info")
                        else:
                    ...
                    if not near_resistance:
                        ...
                        if entry_score_s < 2:
                            ...
                            if should_log_diag_short:
                                ...
                                add_bot_log(user_id, f"💰🔍 {coin}: score suffisant ({entry_score_s}/3) mais momentum pas cohérent ({raison_mom_s}) — pas de vente", "info")
                            ...
                            if should_log_diag_short:
                                ...
                                add_bot_log(user_id, f"💰🔍 {coin}: score suffisant ({entry_score_s}/3) mais ATR ({round(atr_pct_s,3) if atr_pct_s else '?'}%) hors plage cohérente avec le SL ({raison_atr_s}) — pas de vente", "info")
                        elif accumulation_coin_recently_closed(user_id, coin, "SHORT"):
                            add_bot_log(user_id, f"⏳ {coin}: score {entry_score_s}/3 confirmé mais une position Accumulation SHORT vient d'être fermée sur ce coin — cooldown, pas de rachat immédiat", "info")
                        else:
            ...
            if config and "spot_accum_enabled" in config.keys() and config["spot_accum_enabled"] and coin in HL_SPOT_TRADEABLE_COINS and coin not in spot_accum_blocked_set and support and resistance:
                ...
                if spot_channel_pct >= spot_min_channel:
                    ...
                    if near_support_spot:
                        ...
                        # Momentum COHÉRENT — garde-fou de RISQUE séparé (pas une condition de
                        # timing d'entrée), même logique qu'Accumulation.
                        ...
                        spot_price_not_breached = price >= support * (1 - spot_momentum_margin / 100)
                        spot_atr_val = tech.get("atr")
                        spot_candle_atr_mult = config["spot_accum_candle_atr_mult"] if "spot_accum_candle_atr_mult" in config.keys() and config["spot_accum_candle_atr_mult"] is not None else 2.0
                        spot_candle_ok = True
                        if spot_atr_val and candles:
                            spot_last_range = candles[-1]["h"] - candles[-1]["l"]
                            spot_candle_ok = spot_last_range <= spot_atr_val * spot_candle_atr_mult
                        momentum_ok_spot = spot_price_not_breached and spot_candle_ok

                        # SYSTÈME DE SCORE — même principe qu'Accumulation (voir ses
                        # commentaires pour l'explication complète) : au moins 2 des 3
                        # catégories indépendantes, pas de conjonction stricte.
                        spot_vol_mult = config["accumulation_volume_confirm_mult"] if "accumulation_volume_confirm_mult" in config.keys() and config["accumulation_volume_confirm_mult"] is not None else 1.3
                        volume_ok_spot = bool(vol_avg and vol_cur > vol_avg * spot_vol_mult)

                        spot_wick_rejection = bool(len(candles) >= 1 and candles[-1]["l"] <= support and candles[-1]["c"] > support)
                        spot_recent_low = min(closes[-3:]) if len(closes) >= 3 else price
                        spot_bouncing_up = price > spot_recent_low * 1.001
                        spot_rsi_recovering = False
                        if len(closes) > 17:
                            spot_rsi_prev = calc_rsi(closes[:-3], int(rsi_period))
                            if spot_rsi_prev is not None and rsi is not None and rsi > spot_rsi_prev:
                        ...
                        elif not spot_room_ok:
                            add_bot_log(user_id, f"🪙🔍 {coin}: score suffisant ({entry_score_spot}/3) mais trop proche de la résistance ${resistance:.4g} (marge {spot_room_to_resistance:.2f}% < {spot_min_room}%) — pas d'achat spot", "info")
                        else:
        ...
        if accumulation_candidates and config and (config["accumulation_enabled"] or config["accumulation_short_enabled"]):
            ...
            for cand in accumulation_candidates:
                coin, rsi_c, cand_action = cand["coin"], cand["rsi"], cand["action"]
                channel_pct_c = cand.get("channel_pct")
                is_short_cand = cand_action == "SHORT"
                level_c = cand.get("resistance") if is_short_cand else cand.get("support")
                conn_accum = get_db()
                max_pos = (config["accumulation_short_max_positions"] if is_short_cand else config["accumulation_max_positions"]) or 5
                current_pos = conn_accum.execute(
                    "SELECT COUNT(*) FROM paper_trades WHERE user_id=? AND status='OPEN' AND is_accumulation=1 AND action=?", (user_id, cand_action)
                ).fetchone()[0]
                already_here = conn_accum.execute(
                    "SELECT COUNT(*) FROM paper_trades WHERE user_id=? AND coin=? AND status='OPEN' AND is_accumulation=1 AND action=?", (user_id, coin, cand_action)
                ).fetchone()[0]
                ...
                if current_pos >= max_pos or already_here > 0:
                    continue
                # Taille = (capital alloué × 50%) ÷ nombre de positions simultanées max (propre
                # à chaque direction) — même philosophie de compound que le bot principal.
        ...
        # ==================== MODE BREAKOUT — ouverture ====================
        # Même architecture que le traitement Accumulation ci-dessus : sizing 50%/max positions,
        # levier dédié, mode paper/live dédié (breakout_trading_mode) — mais AUCUNE notion de
        # target_pct/support/resistance mémorisés pour une cible (pas de "toucher de cible" en
        # Breakout, voir manage_open_trade). Le niveau cassé est juste conservé pour traçabilité.
        if breakout_candidates and config and (config["breakout_enabled"] or config["breakout_short_enabled"]):
            for cand in breakout_candidates:
                coin, cand_action, level_bo = cand["coin"], cand["action"], cand["level"]
                is_short_bo = cand_action == "SHORT"
                conn_bo = get_db()
                max_pos_bo = config["breakout_max_positions"] or 5
                current_pos_bo = conn_bo.execute(
                    "SELECT COUNT(*) FROM paper_trades WHERE user_id=? AND status='OPEN' AND is_breakout=1", (user_id,)
                ).fetchone()[0]
                portfolio_row_bo = conn_bo.execute("SELECT balance FROM paper_portfolio WHERE user_id=?", (user_id,)).fetchone()
                conn_bo.close()
                if current_pos_bo >= max_pos_bo:
                    continue

                bo_mode = config["breakout_trading_mode"] if "breakout_trading_mode" in config.keys() and config["breakout_trading_mode"] else "paper"
                if bo_mode == "live":
                    conn_bo2 = get_db()
                    user_row_bo = conn_bo2.execute("SELECT hl_wallet FROM users WHERE id=?", (user_id,)).fetchone()
                    conn_bo2.close()
                    hl_wallet_bo = user_row_bo["hl_wallet"] if user_row_bo and "hl_wallet" in user_row_bo.keys() else None
                    capital_bo, hl_err_bo = get_hl_account_value_verbose(hl_wallet_bo)
                    if capital_bo <= 0:
                        raison_bo = hl_err_bo if hl_err_bo else f"solde lu avec succès mais à {capital_bo}$ — compte vide, tout en positions, ou adresse incorrecte"
                        add_bot_log(user_id, f"⛔ {coin}: Breakout live — solde Hyperliquid réel indisponible ({raison_bo}), achat annulé par sécurité", "error")
                        continue
                else:
                    capital_bo = portfolio_row_bo["balance"] if portfolio_row_bo else 1000.0

                bo_size = round((capital_bo * 0.50) / max_pos_bo, 2)
                bo_leverage = (config["breakout_short_leverage"] if is_short_bo else config["breakout_long_leverage"]) or 2
                try:
                    message, _ = execute_manual_trade(user_id, coin, cand_action, bo_size, bo_leverage, {},
                                                        is_breakout=True, breakout_level_price=level_bo, is_manual=False)
                    niveau_label_bo = "résistance" if not is_short_bo else "support"
                    add_bot_log(user_id, f"🤖🚀 Breakout auto {cand_action}: {message} (cassure {niveau_label_bo} ${level_bo:.4g}, volume {cand['vol_ratio']}x)", "success")
                except ValueError as e:
        ...
        if spot_accum_candidates and config and "spot_accum_enabled" in config.keys() and config["spot_accum_enabled"]:
            for cand in spot_accum_candidates:
                ...
                if sp_mode == "live":
                    ...
                    conn_sp2.close()
                    hl_wallet_sp = user_row_sp["hl_wallet"] if user_row_sp and "hl_wallet" in user_row_sp.keys() else None
                    capital_sp, hl_err_sp = get_hl_account_value_verbose(hl_wallet_sp)
                    if capital_sp <= 0:
                    ...
                    capital_sp = portfolio_row_sp["balance"] if portfolio_row_sp else 1000.0
                    size_sp = round((capital_sp * 0.50) / max_pos_sp, 2)
                    if capital_sp < size_sp:
                        continue
                    qty_sp = round(size_sp / cur_price_sp, 6)
                    conn_ins = get_db()
                    conn_ins.execute("""INSERT INTO spot_holdings
                        (user_id, coin, status, entry_price, current_price, qty, size_usdc,
                         support_price, resistance_price, channel_pct, target_pct, is_live,
                        ...
                        VALUES (?,?,'OPEN',?,?,?,?,?,?,?,?,0,?,?)""",
                        (user_id, coin, cur_price_sp, cur_price_sp, qty_sp, size_sp,
                         cand["support"], cand["resistance"], cand["channel_pct"], target_pct_sp,
                         datetime.utcnow().isoformat(), datetime.utcnow().strftime("%Y-%m-%d")))
                    conn_ins.execute("UPDATE paper_portfolio SET balance=balance-? WHERE user_id=?", (size_sp, user_id))
                    conn_ins.commit()
                    conn_ins.close()
                    add_bot_log(user_id, f"🤖🪙 Spot Accumulation (paper): achat {qty_sp} {coin} @ ${cur_price_sp} ({size_sp} USDC, canal {cand['channel_pct']:.1f}%, objectif +{target_pct_sp}%)", "success")
...
ema200_by_coin = {}  # EMA200 par coin, calculée une fois par cycle de scan (change très
                     # lentement, pas besoin de la recalculer à chaque tick) — utilisée par la
                     # sortie simplifiée Accumulation/Spot Accumulation, accessible aussi bien
                     # au cycle de scan qu'aux vérifications temps réel WebSocket/filet 5s.
...
class UpdateConfigRequest(BaseModel):
    ...
    accumulation_arm_pct: Optional[float] = None
    ...
    breakout_short_enabled: Optional[bool] = None
    breakout_max_positions: Optional[int] = None
    breakout_long_leverage: Optional[int] = None
    breakout_short_leverage: Optional[int] = None
    breakout_volume_mult: Optional[float] = None
    breakout_confirm_buffer_pct: Optional[float] = None
    breakout_max_loss_pct: Optional[float] = None
    breakout_trailing_arm_pct: Optional[float] = None
    breakout_trailing_main_pct: Optional[float] = None
    breakout_trailing_gap_pct: Optional[float] = None
    ...
    ttp_giveback_cap_after_pct: Optional[float] = None
    ttp_giveback_cap_pct: Optional[float] = None
...
def get_config(user_id: int = Depends(get_current_user)):
    ...
    return {
        ...
        "accumulation_arm_pct": config["accumulation_arm_pct"] if "accumulation_arm_pct" in config.keys() and config["accumulation_arm_pct"] is not None else 0.4,
        ...
        "breakout_max_positions": config["breakout_max_positions"] if "breakout_max_positions" in config.keys() and config["breakout_max_positions"] else 5,
        "breakout_long_leverage": config["breakout_long_leverage"] if "breakout_long_leverage" in config.keys() and config["breakout_long_leverage"] else 2,
        ...
        "breakout_max_loss_pct": config["breakout_max_loss_pct"] if "breakout_max_loss_pct" in config.keys() and config["breakout_max_loss_pct"] is not None else 0.3,
        "breakout_trailing_arm_pct": config["breakout_trailing_arm_pct"] if "breakout_trailing_arm_pct" in config.keys() and config["breakout_trailing_arm_pct"] is not None else 0.5,
        "breakout_trailing_main_pct": config["breakout_trailing_main_pct"] if "breakout_trailing_main_pct" in config.keys() and config["breakout_trailing_main_pct"] is not None else 1.0,
        "breakout_trailing_gap_pct": config["breakout_trailing_gap_pct"] if "breakout_trailing_gap_pct" in config.keys() and config["breakout_trailing_gap_pct"] is not None else 0.42,
        ...
        "range_trading_max_loss_pct": config["range_trading_max_loss_pct"] if "range_trading_max_loss_pct" in config.keys() and config["range_trading_max_loss_pct"] is not None else 0.3,
        "range_trading_trailing_arm_pct": config["range_trading_trailing_arm_pct"] if "range_trading_trailing_arm_pct" in config.keys() and config["range_trading_trailing_arm_pct"] is not None else 0.5,
        ...
        "ttp_giveback_cap_after_pct": config["ttp_giveback_cap_after_pct"] if "ttp_giveback_cap_after_pct" in config.keys() and config["ttp_giveback_cap_after_pct"] is not None else 3.0,
        "ttp_giveback_cap_pct": config["ttp_giveback_cap_pct"] if "ttp_giveback_cap_pct" in config.keys() and config["ttp_giveback_cap_pct"] is not None else 0.5,
        ...
        "max_loss_pct": config["max_loss_pct"] if "max_loss_pct" in config.keys() and config["max_loss_pct"] else 0.3,
...
def update_config(req: UpdateConfigRequest, user_id: int = Depends(get_current_user)):
    ...
    if req.accumulation_arm_pct is not None:
        conn.execute("UPDATE bot_config SET accumulation_arm_pct=? WHERE user_id=?", (req.accumulation_arm_pct, user_id))
    ...
    if req.breakout_enabled is not None:
        conn.execute("UPDATE bot_config SET breakout_enabled=? WHERE user_id=?", (1 if req.breakout_enabled else 0, user_id))
    if req.breakout_short_enabled is not None:
        conn.execute("UPDATE bot_config SET breakout_short_enabled=? WHERE user_id=?", (1 if req.breakout_short_enabled else 0, user_id))
    if req.breakout_max_positions is not None:
        conn.execute("UPDATE bot_config SET breakout_max_positions=? WHERE user_id=?", (req.breakout_max_positions, user_id))
    if req.breakout_long_leverage is not None:
        conn.execute("UPDATE bot_config SET breakout_long_leverage=? WHERE user_id=?", (req.breakout_long_leverage, user_id))
    if req.breakout_short_leverage is not None:
        conn.execute("UPDATE bot_config SET breakout_short_leverage=? WHERE user_id=?", (req.breakout_short_leverage, user_id))
    if req.breakout_volume_mult is not None:
        conn.execute("UPDATE bot_config SET breakout_volume_mult=? WHERE user_id=?", (req.breakout_volume_mult, user_id))
    if req.breakout_confirm_buffer_pct is not None:
        conn.execute("UPDATE bot_config SET breakout_confirm_buffer_pct=? WHERE user_id=?", (req.breakout_confirm_buffer_pct, user_id))
    if req.breakout_max_loss_pct is not None:
        conn.execute("UPDATE bot_config SET breakout_max_loss_pct=? WHERE user_id=?", (req.breakout_max_loss_pct, user_id))
    if req.breakout_trailing_arm_pct is not None:
        conn.execute("UPDATE bot_config SET breakout_trailing_arm_pct=? WHERE user_id=?", (req.breakout_trailing_arm_pct, user_id))
    if req.breakout_trailing_main_pct is not None:
        conn.execute("UPDATE bot_config SET breakout_trailing_main_pct=? WHERE user_id=?", (req.breakout_trailing_main_pct, user_id))
    if req.breakout_trailing_gap_pct is not None:
        conn.execute("UPDATE bot_config SET breakout_trailing_gap_pct=? WHERE user_id=?", (req.breakout_trailing_gap_pct, user_id))
    if req.breakout_trailing_lock_ratio_pct is not None:
    ...
    if req.range_trading_trailing_main_pct is not None:
        conn.execute("UPDATE bot_config SET range_trading_trailing_main_pct=? WHERE user_id=?", (req.range_trading_trailing_main_pct, user_id))
    if req.range_trading_trailing_gap_pct is not None:
    ...
    if req.spot_accum_trailing_lock_ratio_pct is not None:
        conn.execute("UPDATE bot_config SET spot_accum_trailing_lock_ratio_pct=? WHERE user_id=?", (req.spot_accum_trailing_lock_ratio_pct, user_id))
    if req.spot_accum_momentum_margin_pct is not None:
        conn.execute("UPDATE bot_config SET spot_accum_momentum_margin_pct=? WHERE user_id=?", (req.spot_accum_momentum_margin_pct, user_id))
    if req.spot_accum_candle_atr_mult is not None:
    ...
    if req.ttp_giveback_cap_after_pct is not None:
        conn.execute("UPDATE bot_config SET ttp_giveback_cap_after_pct=? WHERE user_id=?", (req.ttp_giveback_cap_after_pct, user_id))
    if req.ttp_giveback_cap_pct is not None:
        conn.execute("UPDATE bot_config SET ttp_giveback_cap_pct=? WHERE user_id=?", (req.ttp_giveback_cap_pct, user_id))
...
def get_spot_holdings(user_id: int = Depends(get_current_user)):
    conn = get_db()
    open_h = conn.execute(
        "SELECT * FROM spot_holdings WHERE user_id=? AND status='OPEN' ORDER BY opened_at DESC", (user_id,)
    ).fetchall()
    closed_h = conn.execute(
        "SELECT * FROM spot_holdings WHERE user_id=? AND status='CLOSED' ORDER BY closed_at DESC LIMIT 50", (user_id,)
    ).fetchall()
    conn.close()
...
def execute_manual_trade(user_id: int, coin: str, action: str, size_usdc: float, leverage: int, custom: dict, override_paper_price: float = None, is_accumulation: bool = False, accumulation_target_pct: float = None, accumulation_support_price: float = None, accumulation_resistance_price: float = None, accumulation_channel_pct: float = None, is_manual: bool = True, is_breakout: bool = False, breakout_level_price: float = None, is_range_trade: bool = False):
    ...
    if trading_mode == "live":
        ...
        accum_max_loss_for_sl = accum_max_loss_cfg["accumulation_max_loss_pct"] if accum_max_loss_cfg and "accumulation_max_loss_pct" in accum_max_loss_cfg.keys() and accum_max_loss_cfg["accumulation_max_loss_pct"] else 0.3
        breakout_max_loss_for_sl = accum_max_loss_cfg["breakout_max_loss_pct"] if accum_max_loss_cfg and "breakout_max_loss_pct" in accum_max_loss_cfg.keys() and accum_max_loss_cfg["breakout_max_loss_pct"] else 0.3
        range_max_loss_for_sl = accum_max_loss_cfg["range_trading_max_loss_pct"] if accum_max_loss_cfg and "range_trading_max_loss_pct" in accum_max_loss_cfg.keys() and accum_max_loss_cfg["range_trading_max_loss_pct"] else 0.3
...
class SetTakeProfitRequest(BaseModel):
    trade_id: int
    take_profit_pct: Optional[float] = None  # None = retire le TP manuel

@app.get("/api/hl-real-balance")
def get_hl_real_balance(user_id: int = Depends(get_current_user)):
    """Solde réel du compte Hyperliquid — endpoint SÉPARÉ de /api/config (appelé toutes les
    5s) pour ne pas saturer l'API Hyperliquid d'appels inutiles ; à interroger côté frontend
    à un rythme plus espacé (30-60s). Retourne balance=None si aucun mode live n'est actif ou
    si la lecture échoue, pour ne jamais afficher un chiffre trompeur."""
    conn = get_db()
    user = conn.execute("SELECT hl_wallet FROM users WHERE id=?", (user_id,)).fetchone()
    config = conn.execute("SELECT trading_mode, manual_accum_trading_mode, accumulation_short_trading_mode FROM bot_config WHERE user_id=?", (user_id,)).fetchone()
    conn.close()
    any_live = config and (
        config["trading_mode"] == "live" or
        (config["manual_accum_trading_mode"] if "manual_accum_trading_mode" in config.keys() else None) == "live" or
...
def set_take_profit(req: SetTakeProfitRequest, user_id: int = Depends(get_current_user)):
    """Définit ou modifie un Take Profit manuel (% de PnL, avec effet de levier — cohérent avec
    le PnL affiché sur la carte du trade) sur un trade DÉJÀ OUVERT — fonctionne pour tous les
    modes (bot principal, manuel, Accumulation LONG/SHORT) puisque tous passent par le même
    champ custom_take_profit_pct, vérifié en priorité dans manage_open_trade."""
    conn = get_db()
